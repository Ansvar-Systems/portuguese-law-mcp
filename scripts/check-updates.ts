#!/usr/bin/env tsx
/**
 * Portuguese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://dre.pt';
const PORTAL_NAME = 'Diario da Republica Eletronico';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/portuguese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Portuguese Law MCP — Data Freshness Check');
