/**
 * get_portuguese_implementations — Find Portuguese statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetPortugueseImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface PortugueseImplementationResult {
  document_id: string;
...
export async function getPortugueseImplementations(
  db: InstanceType<typeof Database>,
  input: GetPortugueseImplementationsInput,
): Promise<ToolResponse<PortugueseImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as PortugueseImplementationResult[];
