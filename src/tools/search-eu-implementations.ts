/**
 * search_eu_implementations — Search EU directives/regulations referenced by Portuguese legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_portuguese_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  portuguese_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as portuguese_statute_count
  ...
  if (input.has_portuguese_implementation) {
    sql += ' HAVING portuguese_statute_count > 0';
  }
